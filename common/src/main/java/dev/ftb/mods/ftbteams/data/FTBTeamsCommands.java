				...
				.then(Commands.literal("party")
						...
						.then(Commands.literal("war")
								...
								.then(Commands.literal("declare")
										.requires(source -> hasParty(source, TeamRank.OFFICER))
										.then(teamArg()
												.executes(ctx -> getPartyTeam(ctx, TeamRank.OFFICER).declareWar(ctx.getSource(), teamArg(ctx)))
										)
								)
								.then(Commands.literal("end")
										.requires(source -> hasParty(source, TeamRank.OFFICER))
										.then(teamArg()
											.executes(ctx -> getPartyTeam(ctx, TeamRank.OFFICER).declarePeace(ctx.getSource(), teamArg(ctx)))
										)
								)
								.then(Commands.literal("list")
										.requires(source -> hasParty(source, TeamRank.MEMBER))
										.executes(ctx -> getPartyTeam(ctx, TeamRank.MEMBER).listWars(ctx.getSource()))
								)
						)
